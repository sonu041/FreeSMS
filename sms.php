...
class WAY2SMSClient
{
    ...
    function login($username, $password)
    {
        ...
        curl_setopt($this->curl, CURLOPT_USERAGENT, "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.140 Safari/537.36");
...
if (isset($_REQUEST['uid']) && isset($_REQUEST['pwd']) && isset($_REQUEST['phone']) && isset($_REQUEST['msg'])) {
    ...
    if (is_array($res))
			echo $res[0]['result'] ? "SMS Sent to: " .$_REQUEST['phone'] ."<br/>Content: ".$_REQUEST['msg'] : 'error';
    else
      echo $res;
    exit;
}
